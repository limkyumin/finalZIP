...
<body id="page-top">
	<div id="wrapper">
		...
		<div align="center" style="width:120%; height:50px;">
			<form id="searchForm"  action="searchYmem.mem" method="get" >
    		...
    		<c:if test="${pi.currentPage ne 1 }">
    			<button class="btn btn-primary" onclick="location.href='todayYmember.li?mpage=${pi.currentPage-1}'">[이전]</button>
    		</c:if>
            <c:forEach var="i" begin="${pi.startPage}" end="${pi.endPage}" step="1">
	 			<c:choose>
	 				<c:when test="${ empty map }">
	 					<button class="btn btn-primary" onclick="location.href='todayYmember.li?mpage=${i}'">[${i}]</button>
	 				</c:when>
	 				
	 				<c:otherwise>
				  		<button class="btn btn-primary" onclick="location.href='searchYmem.mem?mpage=${i}&type=${type}&keyword=${keyword}'">[${i}]</button>
	 				</c:otherwise>
	 				
	            </c:choose>
            </c:forEach>
            <c:if test="${ pi.currentPage ne pi.maxPage }">
            	<button class="btn btn-primary" onclick="location.href='todayYmember.li?mpage=${pi.currentPage+1}'">[다음]</button>
            </c:if>
    	</div>
